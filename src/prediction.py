...
# --- 3. UI RENDERING ---
def run():
    # Injeksi CSS buat input selaras & transisi halus
    st.markdown("""
    <style>
    .input-container {
        margin-top: 1.5rem;
    ...
    st.markdown('<p style="color: #94a3b8; margin-bottom: 5px;">Pilih Kategori Produk</p>', unsafe_allow_html=True)
    selected_kat = st.selectbox("", list(meta['final_recipes'].keys()), label_visibility="collapsed")

    # Animasi Square
    st.markdown('<div style="display: flex; justify-content: center; margin: 15px 0;"><div class="square" style="width: 40px; height: 40px; background: linear-gradient(45deg, #3b82f6, #ec4899); border-radius: 10px;"></div></div>', unsafe_allow_html=True)
    ...
    if st.button("Run Hybrid Prediction 🚀", use_container_width=True, type="primary"):
        with st.status(f"AI sedang menganalisis kategori {selected_kat}...", expanded=True) as status:
            ...
            time.sleep(1)
            st.write("Running Hybrid Engine...")
            time.sleep(1)
