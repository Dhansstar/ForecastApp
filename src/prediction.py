...
# --- 1. SMART PATHING (Dongo-Proof) ---
# Kita cari tahu kita lagi di mana
BASE_PATH = os.path.dirname(os.path.abspath(__file__))

# Coba cari folder model di tempat script ini berada (src/)
MODEL_DIR = os.path.join(BASE_PATH, 'models_demandsense_v1')

# Kalau ternyata script jalan dari root (di luar src), kita arahin masuk ke src
if not os.path.exists(MODEL_DIR):
    MODEL_DIR = os.path.join(BASE_PATH, 'src', 'models_demandsense_v1')

@st.cache_resource
def load_assets():
    """Load model & metadata dengan pengecekan jalur ganda."""
    meta_path = os.path.join(MODEL_DIR, 'model_metadata.json')
    
    if not os.path.exists(meta_path):
        st.error(f"❌ TETEP GAK KETEMU, JING!")
        st.write(f"Server nyari di: {meta_path}")
        st.write("Daftar folder yang ada di lokasi script:", os.listdir(BASE_PATH))
        st.stop()
        
    with open(meta_path, 'r') as f:
        meta = json.load(f)
    
    # Load Model (Recursive Style)
...
def load_and_sync_data():
    """Baca CSV biar angka stok Kitchen 1107 & Home 860."""
    files = {
    ...
    for kat, fname in files.items():
        # Cari CSV di folder yang sama sama script ini
...
# --- 2. RECURSIVE ENGINE (IDENTIK NOTEBOOK) ---
def run_recursive_forecast(kat, meta, fe_model, vol_model, mape_model, full_df):
    ...
    for i in range(1, 31):
        ...
        new_row = [
            ...
            np.log1p(val), # log transform lag_1
...
def run_prediction():
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=forecast_dates, y=daily_preds, name="Forecast", line=dict(width=3, color='orange')))
