...
def load_assets():
    try:
        ...
        # PASTIKAN lo udah upload file .keras hasil konversi di lokal!
...
# --- 2. ENGINE RECURSIVE (Fungsi yang tadi ilang) ---
def run_recursive_forecast(kat, meta, fe_model, vol_model, mape_model, full_df):
    ...
    # Padding jika data historis kurang
...
def run_prediction():
    ...
    if not all_dfs:
        st.error("Data historis (CSV) tidak ditemukan di folder src/!")
        st.stop()
    ...
    st.markdown("""
        <div style="background-color:#1e293b; padding:15px; border-radius:10px; border-left: 5px solid #3b82f6; margin-bottom:20px;">
            ...
            <p style="color:white; margin:0;">Pilih kategori produk untuk melihat estimasi stok 30 hari ke depan.</p>
        </div>
