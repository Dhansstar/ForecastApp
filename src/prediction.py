...
# --- 3. UI RENDERING ---
def run():
    # CSS ANTI-KOTAK KOSONG & SMOOTH UI
    st.markdown("""
    <style>
    /* Ngilangin gap antar elemen markdown */
    .element-container, .stMarkdown { margin-bottom: 0px !important; }
    ...
    /* Paksa label selectbox ilang biar gak nyampah spasi */
    ...
    # --- WRAPPER INPUT (Kunci Layout Bersih) ---
    st.markdown('<div class="input-wrapper animate-card">', unsafe_allow_html=True)
    
    st.markdown('<p style="color: #94a3b8; font-weight: 600; margin-bottom: 8px;">Pilih Kategori Produk</p>', unsafe_allow_html=True)
    selected_kat = st.selectbox("label_hidden", list(meta['final_recipes'].keys()), label_visibility="collapsed")

    # Animasi Square Tengah
    ...
    btn_clicked = st.button("Run Hybrid Prediction 🚀", use_container_width=True, type="primary")
