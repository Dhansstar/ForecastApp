...
def load_assets():
    try:
        ...
        # Pake format .keras (Pastikan file sudah ada di folder src/)
...
def run_recursive_forecast(kat, meta, fe_model, vol_model, mape_model, full_df):
    ...
    # Ambil Window Awal & Paksa Dimensi Tiap Baris (PENTING!)
    hist_input = hist.tail(time_steps)
    # Kita paksa tiap baris di curr_win cuma punya panjang expected_f
    ...
    preds = []
    for i in range(1, 31):
        # Sekarang np.array gak akan error karena panjang list di dalamnya seragam
        ...
        # Update Window untuk hari berikutnya
        nxt_d = last_date + pd.Timedelta(days=i)
        new_row = [
            np.sin(2*nxt_d.day*np.pi/31), 
            np.cos(2*nxt_d.day*np.pi/31), 
            np.log1p(val),
        ...
        # Gabungkan fitur baru + kategori, lalu potong agar pas expected_f
...
def run_prediction():
    ...
    st.markdown("""
        <div style="background-color:#1e293b; padding:15px; border-radius:10px; border-left: 5px solid #3b82f6; margin-bottom:20px;">
            <h3 style="color:#3b82f6; margin:0;">🔮 DemandSense AI - Forecasting</h3>
            <p style="color:white; margin:0;">Aplikasi prediksi stok berbasis AI.</p>
        </div>
